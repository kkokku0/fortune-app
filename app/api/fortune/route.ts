...
- 문제는 벌기도 전에 샐 구멍부터 만들어놓는다는 거야.
- 충동구매, 남 말 듣고 움직이는 돈, 정 때문에 나가는 돈. 이 세 개 못 잡으면 벌어도 남는 게 적다.
- 대박보다 먼저 돈이 남는 구조를 만들어야 한다.
`;
  }
  ...
  if (categoryId === "career" || title.includes("직업") || title.includes("사업")) {
    return `
[카테고리 톤: 직업/사업운]
- 너는 능력이 없는 게 아니다. 생각만 많고 실행이 늦어지는 게 문제다.
- 이 패턴 못 끊으면 계속 뭐 해먹고 살지 고민만 반복한다.
- 너는 시키는 일만 오래 버티는 팔자보다, 방향을 잡고 판을 바꿀 때 일이 풀리는 사주다.
- 직장, 부업, 사업을 현실적으로 나눠서 말해라.
`;
  }
  ...
  if (categoryId === "love" || title.includes("연애")) {
    return `
[카테고리 톤: 연애운]
- 너는 연애운이 없는 게 아니다. 외로울 때 사람 보는 눈이 흐려지는 쪽이다.
- 상대가 애매한데도 네가 의미를 붙이면 그건 운명이 아니라 감정노동 시작이다.
- 불안을 설렘으로 착각하면 또 같은 상처 반복한다.
- 새 인연, 썸, 재회, 기존 관계 흐름을 연애운 안에서만 풀어라.
`;
  }
  ...
  if (categoryId === "compatibility" || title.includes("궁합")) {
    return `
[카테고리 톤: 궁합풀이]
- 이 관계는 편한 궁합이라기보다 서로의 약점을 정확히 건드리는 궁합이다.
- 끌림은 있는데 회복 방식이 다르면 계속 피곤해진다.
- 본인과 상대방의 기질을 반드시 비교해라.
- 맞는 부분, 안 맞는 부분, 부딪히는 이유, 앞으로의 관계 흐름을 나누어라.
`;
  }
  ...
  if (categoryId === "family" || title.includes("가족")) {
    return `
[카테고리 톤: 가족관계]
- 가족이라고 다 감당하는 게 네 팔자는 아니다.
- 네가 계속 참으면 상대가 고마워하는 게 아니라 당연하게 여길 수 있다.
- 정을 끊으라는 게 아니라 선을 잡으라는 흐름으로 풀어라.
- 가족 안에서의 역할, 상대방 기질, 반복되는 서운함, 거리 조절을 말해라.
`;
  }
  ...
  if (categoryId === "traditional" || title.includes("평생종합사주")) {
    return `
[카테고리 톤: 평생종합사주]
- 너는 망한 사주가 아니다. 근데 편하게 풀리는 사주도 아니다.
- 초반보다 뒤로 갈수록 흐름이 잡히는 사주인지, 빨리 풀리는 사주인지 분명히 말해라.
...
`;
  ...
  return `
[카테고리 톤: 공통]
- 너는 운이 없는 게 아니라, 운을 쓰는 방식이 꼬인 쪽에 가깝다.
- 좋은 말로 포장하지 말고, 고쳐야 할 패턴을 분명히 말해라.
- 단점은 세게 말하되 모욕하지 마라.
...
팩폭 규칙:
- 사용자의 단점을 돌려 말하지 마라.
- 게으름, 미룸, 핑계, 눈치, 집착, 충동, 돈 새는 습관, 사람 보는 눈 같은 단어를 상황에 맞게 써라.
- 단, 모욕하거나 비하하지 마라.
- 단점은 반드시 "고치지 않으면 반복될 결과"와 함께 말해라.
- 그리고 반드시 "고치면 어떻게 풀리는지"도 말해라.
- 각 섹션에 최소 1개는 한방 문장을 넣어라.
...
- 다른 카테고리 추천 금지.
- 선택된 카테고리만 깊게 풀어라.
- CHAPTER 01~03에 해당하는 앞부분은 짧고 강하게.
- 그 뒤는 충분히 길고 깊게 풀어라.
...
- 초년운, 청년운, 중년운, 말년운 전체를 다 풀지 말고 힌트만 보여줘라.
- 짧고 강하게 써라.
- 문단은 짧게 써라.
- "이번 달" 표현 금지.
...
[타고난 운명 기본 구조]
강한 기운, 약한 기운, 반복되는 인생 패턴을 짧게 풀어라.
...
- CHAPTER 01~03에 해당하는 앞부분은 짧고 임팩트 있게.
- 그 뒤는 충분히 길고 깊게 풀어라.
- 각 섹션 첫 문장은 강하게 써라.
- 고쳐야 할 문제를 적나라하게 말해라.
...
비유를 하나 넣어도 좋다. 예: 전생에 장부를 들고 다니던 사람 같은 기운.
...
문제는 생각만 많고 실행이 늦어지는 순간이 있다는 거야.

이거 못 고치면 같은 고민만 계속 반복할 수 있어.
...
전체 리포트에서는 ${categoryTitle} 안에서 막히는 패턴, 3개월 흐름, 1년 방향을 더 깊게 이어서 볼 수 있어.`;
}
    ...
    if (mode === "preview") {
      ...
      try {
        preview = await generateText(
          ...
          categoryId === "traditional" ? 1800 : 1900
        );
      } catch (error) {
    ...
    if (mode === "full") {
      ...
      try {
        full = await generateText(
          ...
          categoryId === "traditional" ? 6200 : 5200
        );
      } catch (error) {
...
export async function GET() {
  return NextResponse.json({
    ok: true,
    message: "soreum saju punch fortune api is working",
    model: MODEL,
  });
}